...
def _query_cache_key(tenant_id: str, query: str) -> str:
    h = hashlib.md5(f"{tenant_id}:{query}".encode("utf-8")).hexdigest()
    return h
...
async def chat(request: Request, body: ChatRequest):
    ...
    cache_key = _query_cache_key(body.tenant_id, body.query)
